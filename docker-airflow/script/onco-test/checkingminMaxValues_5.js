...
var filestream = function(fs){
  return new Promise(function(resolve, reject){
  var stream = fs.createWriteStream("/usr/local/airflow/docker-airflow/onco-test/CheckingMinMaxValues_5.json",{  
        flags: 'w',
...
Promise.all([mongo(mongoose),filestream(fs)]).then(function(response){
    ...
    manifest_molecular = manifest_molecular.splice(400, 100);
