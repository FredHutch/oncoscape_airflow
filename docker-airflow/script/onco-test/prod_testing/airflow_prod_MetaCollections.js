...
co(function *() {
  ...
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-allCollectionNames.json", allCollectionNames, function(err){console.error(err);});
  
  collection = yield comongo.db.collection(db, "manifest");
  var manifest_arr = yield collection.find({}).toArray();
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-manifest_all.json", manifest_arr, {spaces: 2}, function(err){console.error(err);});
  ...
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-manifest_arr.json", manifest_arr, {spaces: 2}, function(err){ console.error(err);});  
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-lookup_arr.json", lookup_arr, {spaces: 2}, function(err){ console.error(err);});  
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-render_pca.json", render_pca, {spaces: 2}, function(err){ console.error(err);});  
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-render_patient.json", render_patient, {spaces: 2}, function(err){ console.error(err);});  
  jsonfile.writeFile("/usr/local/airflow/docker-airflow/onco-test/prod-lookup_dataTypes.json", lookup_dataTypes, {spaces: 2}, function(err){ console.error(err);});  
